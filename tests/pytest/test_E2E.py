...
def E2E_run(inventoryPage, error_list, test_case):
    '''
    ## Test inventory page
    ...
    try:
        ...
        error_list.append(f"Unexpected Alert. {e.alert_text}")
...
@pytest.mark.parametrize("username", ["standard_user", "problem_user", "error_user"])
def test_E2E(username, setup_teardown):
    error_list = []
    test_case = username
    ...
    try: 
        error_list = E2E_run(inventoryPage=inventoryPage,
                             error_list=error_list,
                             test_case=test_case
                             )
    except Exception as e:
        utils.takeScreenshot(test_case=test_case, driver=inventoryPage.driver)
        inventoryPage.logout()
        error_list.append(f"Error performing end to end test. {str(e)}")
